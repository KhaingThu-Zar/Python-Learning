odoo.define('test_base_import.import', function (require) {
"use strict";
...
function jsonp(form, attributes, callback) {
	alert("I am an alert box! within Jsonp");
...
var DataImport = AbstractAction.extend(ControlPanelMixin, {
    ...
    events: {
        ...
        'change .oe_test_import_file': 'loaded_file',
    ...
    renderButtons: function() {
    	alert("I am an alert box!4");
        var self = this;
        this.$buttons = $(QWeb.render("TestImportView.buttons", this));
        this.$buttons.filter('.o_test_import_validate').on('click', this.validate.bind(this));
        this.$buttons.filter('.o_test_import_import').on('click', this.import.bind(this));
        this.$buttons.filter('.o_test_import_file_reload').on('click', this.loaded_file.bind(this));
        this.$buttons.filter('.oe_test_import_file').on('click', function () {
            self.$('.oe_test_import_file').click();
        });
        this.$buttons.filter('.o_test_import_cancel').on('click', function(e) {
            e.preventDefault();
            self.exit();
        });
    },
    ...
    onfile_loaded: function () {
    	alert("I am an alert box!7");
        this.$buttons.filter('.o_test_import_import, .o_test_import_validate, .o_test_import_file_reload').addClass('d-none');
        if (!this.$('input.oe_test_import_file').val()) { return this['settings_changed'](); }
        this.$('.oe_import_date_format').select2('val', '');
        this.$('.oe_import_datetime_format').val('');

        this.$el.removeClass('oe_import_preview oe_import_error');
        var import_toggle = false;
        var file = this.$('input.oe_test_import_file')[0].files[0];
    ...
    onpreview_error: function (event, from, to, result) {
    	alert("I am an alert box!9");
        this.$('.oe_import_options').show();
        this.$buttons.filter('.o_test_import_file_reload').removeClass('d-none');
        ...
        this.$('.oe_import_error_report').html(
                QWeb.render('TestImportView.preview.error', result));
    },
    onpreview_success: function (event, from, to, result) {
    	alert("I am an alert box!10");
        var self = this;
        this.$buttons.filter('.oe_test_import_file')
            .text(_t('Load New File'))
            .removeClass('btn-primary').addClass('btn-secondary')
            .blur();
        this.$buttons.filter('.o_test_import_import, .o_test_import_validate, .o_test_import_file_reload').removeClass('d-none');
        this.$el.find('.oe_import_box, .oe_import_with_file').removeClass('d-none');
        this.$el.find('.o_view_nocontent').addClass('d-none');
        this.$el.addClass('oe_import_preview');
        this.$('input.oe_import_advanced_mode').prop('checked', result.advanced_mode);
        this.$('.oe_import_grid').html(QWeb.render('TestImportView.preview', result));
        ...
        $fields.each(function (k,v) {
            ...
            var bind = function (d) {};
            if (session.debug) {
                $thing = $(QWeb.render('TestImportView.create_record_option')).insertAfter(v).hide();
                bind = function (data) {
                    switch (data.type) {
    ...
    onresults: function (event, from, to, results) {
        ...
        this.$('.oe_import_error_report').html(
            QWeb.render('TestImportView.error', {
